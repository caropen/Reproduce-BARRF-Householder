...
"""Create a manuscript PDF of rangefinder runtimes through n=2048."""
...
SERIES = (
    SeriesSpec(
        "cpu.csv", "k", "CPU non-adaptive", "#d62728", "D"
    ),
    SeriesSpec(
        "magma_gpu.csv", "k", "MAGMA non-adaptive", "#1f77b4", "o"
    ),
    SeriesSpec(
        "cusolver_gpu.csv", "k", "cuSOLVER non-adaptive", "#ff7f0e", "s"
    ),
    SeriesSpec(
...
OUTPUT_NAME = "rangefinder_runtimes_upto_2048.pdf"
MAX_X = 2048
FULL_X_TICKS = (
    # 128,
    # 256,
...
def load_series(csv_file: Path, x_column: str) -> tuple[list[int], list[float]]:
    ...
    if not points:
        raise ValueError(
            f"{csv_file.name} has no successful runtimes at or below {MAX_X}"
        )
    points.sort()
...
def main() -> int:
    ...
    axis.set_title(
        f"Rangefinder runtime, $M=N=16384${title_suffix}", fontsize=8.5
    )
    axis.set_xlabel(
