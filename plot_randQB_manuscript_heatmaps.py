...
PANEL_TITLES = {
    "randqb_lapack": "(a) RSVDpack, CPU",
    "randqb_cublas": "(b) RSVDpack, GPU",
}
...
def parse_arguments() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        default=script_directory / "benchmark_results" / "legacy_qb",
        help=(
            "directory containing the two benchmark CSV files "
            "(default: benchmark_results/legacy_qb beside this script)"
        ),
    )
...
def load_grid(csv_file: Path, pd, np):
    ...
    if actual_grid != expected_grid:
        raise ValueError(f"{csv_file.name} does not contain the expected grid")
